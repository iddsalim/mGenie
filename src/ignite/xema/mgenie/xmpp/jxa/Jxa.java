...
package ignite.xema.mgenie.xmpp.jxa;
...
/*	public Jxa(final String host, final String port, final String username, final String password, final String resource, final int priority) {
	...
	//public void saveContact(final String jid, final String name, final Enumeration group, final String subscription) {
        public void saveContact(final String jid, final String name, final String group, final String subscription) {
		try {
                        if (DEBUG) java.lang.System.out.println("ADDING: " + jid + " ("+ name +")... Group : "+ group + " - Subs : subscription");
			this.writer.startTag("iq");
			this.writer.attribute("type", "set");
			this.writer.startTag("query");
			...
			/*if (group != null) {
				while (group.hasMoreElements()) {
					...
					this.writer.endTag(); // group
                        ...
                        // For now, user can only belong to one group...
                        this.writer.startTag("group");
	...
	private void parse() throws IOException {
		if (DEBUG) java.lang.System.out.println("*debug* parsing START_______");
		if (!use_ssl) { this.reader.next(); } // start tag
		while (this.reader.next() == XmlReader.START_TAG) {
			final String tmp = this.reader.getName();

                        System.out.println("|>> PARSE - Parsed : " + tmp );
			...
			if (tmp.equals("message")) {
                            ...
                            this.parseIq();
			} else if (tmp.equals("items"))     {   // Ignore for now...
                            java.lang.System.out.println("Ignoring 'ITEMS'...");
			} else if (tmp.equals("activities")) {  // Ignore for now...
                            java.lang.System.out.println("Ignoring 'ACTIVITIES'...");
			} else {
                            this.parseIgnore();
			}
		}
		java.lang.System.out.println("leave parse() " + reader.getName()+ "_________");
		this.reader.close();
	}
	...
	private void parseIq() throws IOException {
		...
		if (type.equals("error")) {
		...
		} else if (type.equals("result") && (id != null) && (id.equals("res_binding") || id.equals("auth"))) {
                    if (DEBUG) java.lang.System.out.println("*debug* Login OK");
			// authorized OK
			while (true) {
                            reader.next();
			...
			java.lang.System.out.println("ELSE : XML :"+ this.reader.toString());
			while (this.reader.next() == XmlReader.START_TAG) {
                ...
                //String pic = reader.getAttribute("image");
                ...
                while (this.reader.next() == XmlReader.START_TAG) {
                    ...
                    //if (check) {
                          for (Enumeration e = listeners.elements(); e.hasMoreElements();) {
                            XmppListener xl = (XmppListener) e.nextElement();
                            xl.onContactEvent(jid, name, "Friends", subscription);
                          }
                    }
                } else {
                        this.parseIgnore();
	...
	private void parsePresence() throws IOException {
                ...
                System.out.println("PARSE PRESENCE STARTED...");

		//final String from = this.reader.getAttribute("from"), type = this.reader.getAttribute("type");
                String from = this.reader.getAttribute("from"), type = this.reader.getAttribute("type");
		String status = "", show = "",  photoStr = "";
		int priority=-1;
		while (this.reader.next() == XmlReader.START_TAG) {
			final String tmp = this.reader.getName();
                        final int intType = this.reader.getType();
			if (tmp.equals("status")) {
                        ...
                        //} else if (intType==2 ) {
                            //} else if (intType==4 && (tmp==null || tmp=="") ) {
                            //photoStr = this.reader.getText();//this.parseText();
                            //StrPhoto = "";
                            //photoStr = this.parseIgnoreStr();//this.reader.getText();//this.parseText();
			} else {
                            System.out.println("IGNORING :::::> " + intType +"---@---"+ tmp +"...");
                            this.parseIgnore();
			}
                        //System.out.println("Parsed : " + tmp.toString() + "... TYPE : " + type + ", FROM :" + from + ", SHOW :" + show + ", STATUS :" + status);
		}

		//if (DEBUG) java.lang.System.out.println("*debug* -parsePresence- from,type,status,show:" + from + "," + type + "," + status + "," + show);
                if (DEBUG) java.lang.System.out.println("***PHOTO : " + photoStr);
		...
		if (type == null || type.equals("") ) {
                ...
                System.out.println("PARSE PRESENCE ENDED...");
	}
	...
	private void parseMessage() throws IOException {
		...
		// (from, subject, body);
                if (body.toString().equals("") ) { // || subject.equals("")) {
                    //System.out.println("\n:::> [MSG_NACK] body, subj [" + body + "," + subject + "]" );
                } else {
                    //System.out.println("\n:::> [MSG_ACK");
                    for (Enumeration e = listeners.elements(); e.hasMoreElements();) {
                        XmppListener xl = (XmppListener) e.nextElement();
	...
	private void parseIgnore() throws IOException {
            int x;
                ...
                //System.out.println("**** BEGIN PARSE IGNORE ****");

            while ((x = this.reader.next()) != XmlReader.END_TAG) {
                System.out.println("**** GOT [getName,getText,getType] - ["+ this.reader.getName() + ","+ this.reader.getText() + ","+ this.reader.getType() + "] @ PARSE IGNORE ****");
                if (x == XmlReader.START_TAG) {
                    System.out.println("**** GOT x as : " + x);
                    this.parseIgnore();
                }
            }
                //System.out.println("**** END PARSE IGNORE ****");
	}

      	private String parseIgnoreStr() throws IOException {
            int x;

            System.out.println("**** BEGIN PARSE IGNORE **** - StrPhoto = ["+ StrPhoto+"]");

            while ((x = this.reader.next()) != XmlReader.END_TAG) {
                String tx, n;
                int ty;
                n = this.reader.getName();
                tx = this.reader.getText();
                ty = this.reader.getType();

                System.out.println("**** GOT [getName,getText,getType] - ["+ n + ","+ tx + ","+ ty + "] @ PARSE IGNORE ****");

                if (StrPhoto.equals("")) {
                    if (ty==4){
                        StrPhoto = tx.toString();
                        //break;
                    }
                }else {
                    return StrPhoto;//"XXXXXX";
                }
                if (x == XmlReader.START_TAG && StrPhoto.equals("")) {  this.parseIgnoreStr();  }
            }
